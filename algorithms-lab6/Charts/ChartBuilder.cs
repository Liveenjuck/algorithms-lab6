...
public static class ChartBuilder {
    ...
    public static string Build2DLineChart(ChartData cd, string outputDir, bool promptOnOverwrite = true) {
        Console.WriteLine($"{cd.Title} – {cd.TotalExecTimeSeconds}s");

        if (string.IsNullOrWhiteSpace(outputDir)) {
            throw new ArgumentException("Output directory must be specified.", nameof(outputDir));
        }
